...
class Scamper:
  def __init__(self, outfile=None, host='localhost', port=31337, verbose=False):
    ...
    if outfile:
      try:
        ...
        print(f"Couldn't open warts file: {e}")
        sys.exit(-1)
  ...
  def fetch_scamper_result(self):
    ...
    while True:
      ...
      while True:
        ...
        if line.find(b'ERR') != -1:
          print("** error response:", line)
          raise Exception('shit, talk to rob')
        elif line.find(b'MORE') != -1:
          continue
        elif line.find(b'DATA') != -1:
          break
